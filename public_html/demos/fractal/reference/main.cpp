...
void buddha()
{
	...
	int r,c,i,rr,cc,historyLength=10;
	std::list<int> tmpr,tmpc;
		...
		for (c=0;c<width;c++)
		{
			...
			if (test(a,b,r,c))
			{
				...
				for (i=1;i<iter && x2+y2<4;i++)
				{
					...
					rr = y*(h4)+(h2);
					cc = x*(w4)+(w2);
					std::list<int>::iterator itrr = tmpr.begin();
					std::list<int>::iterator itrc = tmpc.begin();
					for (;itrr != tmpr.end() && itrc != tmpc.end();++itrr, ++itrc)
					{
						if (*itrr == rr && *itrc == cc)
							stop = true;
					}
